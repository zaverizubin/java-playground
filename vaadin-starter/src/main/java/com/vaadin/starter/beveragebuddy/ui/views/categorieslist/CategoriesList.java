...
public class CategoriesList extends VerticalLayout {
	...
	private void addSearchBar() {
		...
		final Button newButton = new Button("New category", new Icon("lumo", "plus"));
	...
	private Button createEditButton(final Category category) {
		final Button edit = new Button("Edit", event -> form.open(category,
	...
	private String getReviewCount(final Category category) {
		final List<Review> reviewsInCategory = ReviewService.getInstance()
				.findReviews(category.getName());
		final int sum = reviewsInCategory.stream().mapToInt(Review::getCount).sum();
		return Integer.toString(sum);
	}

	private void updateView() {
		final List<Category> categories = CategoryService.getInstance()
	...
	private void saveCategory(final Category category,
			final AbstractEditorDialog.Operation operation) {
